...
def apply_color_tags(treeview_id, colors_hex):
    for i, color_hex in enumerate(colors_hex):
        ...
        softened_rgb = tuple(int((rgb_color[j] + 255) / 2) for j in range(3))
        softened_color_hex = f'#{softened_rgb[0]:02x}{softened_rgb[1]:02x}{softened_rgb[2]:02x}'
        treeview.tag_configure(tag_name, background=softened_color_hex)
        brightness = (0.2126 * rgb_color[0] + 0.7152 * rgb_color[1] + 0.0722 * rgb_color[2]) / 255
        if brightness < 0.5:
            treeview.tag_configure(tag_name, foreground="white")
        else:
            softened_text_color = f'#{int(255 * (1 - brightness)):02x}{int(255 * (1 - brightness)):02x}{int(255 * (1 - brightness)):02x}'
            treeview.tag_configure(tag_name, foreground=softened_text_color)
